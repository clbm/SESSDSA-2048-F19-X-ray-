Min_mark = -1<<20
Max_Mark = 1<<20
Dict = {1:0, 2:0, 3:0, 4:1, 5:1, 6:3, 7:5, 8:9, 9:20, 10:50, 11:200}
...
def getmark(board,isFirst):
    ...
    ourN=board.getNone(isFirst)
    theirN=board.getNone(not isFirst)
    result = -(len(ours)*2)+len(theirs)   #我方棋子越少越好
    if (4,1) in theirN or (4,2) in theirN:  #这两个区域风险系数很大
        result-=10
    if (3,1) in ourN or (3,2) in ourN:
        result-=5
    result+=Dict[board.getValue((0,0))]+Dict[board.getValue((0,3))]  #两角值越大，局面越有利（过小时应排除这一项）
    
    result+=sum(ours[:-1])+Dict[ours[-1]]
    result-=sum(theirs[:-1])+Dict[theirs[-1]]
