...
def getmark(board,isFirst,Round):
    '评估函数'
    #part1:棋子直接估值部分
    #获得两方棋子数值的从大到小的列表
    ours = board.getScore(isFirst)
    theirs = board.getScore(not isFirst)
    #our_spaces = len(board.getScore(isFirst))
    #their_spaces = len(board.getScore(not isFirst))
    if Round > 20:
        ...
        for chess in theirs:
            result += 1<<(14-chess)
        return result                  #+ our_spaces - their_spaces
    else:
        #zeros = 32-len(ours)-len(theirs)
        #result = zeros*(1<<14)
        result = -1
...
def simulation(depth:int, Round:int, Gamemode:int, is_max:bool, board, alpha = Min_mark, beta = Max_Mark, is_root = False):
    ...
    if depth >= max_depth or Round == 500:
        alpha = beta = getmark(board,isFirst,Round)
        return (alpha, beta)
