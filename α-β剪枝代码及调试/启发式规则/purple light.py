...
class Net_p(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(2, 40, (4,1))
        self.pool1 = nn.MaxPool2d((1, 2))
        self.conv2 = nn.Conv2d(2,80,(1,8))
    ...
    def forward(self, x):
        ...
        return self.fc(x)
net = Net_p()
net.load_state_dict(torch.load('./model_3'))
...
class Player:
    ...
    def output(self, currentRound:int, board, mode:str):
        ...
        if mode == 'position':  # 给出己方下棋的位置
            BD = numpy.array(board.getRaw()).transpose((2,0,1))
            BD[1] -= 1^BD[1]
            mine = board.getNext(self.isFirst, currentRound)
            if net(torch.FloatTensor(BD))[0][0] > 0.6 and mine:
                return mine
