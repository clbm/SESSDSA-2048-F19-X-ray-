...
Max_Mark = 1<<20
#不同位置权重
POS = {1:{0: 18, 1: 18, 2: 19, 3: 19, 4: 21, 5: 21, 6: 22, 7: 22},0:{0: 22, 1: 22, 2: 21, 3: 21, 4: 19, 5: 19, 6: 18, 7: 18}}
#不同棋子权重
...
def getmark(board):
    ...
    for i in range(4):
        for j in range(8):
            value,belong = Array[i][j]
            if value: #非0情况
                result += POS[belong][j]*Dict[value]*(2*belong-1) #2*x-1可以把0,1变成-1,1
            else:
                result += 40*belong-20 #0的情况
    return result*(2*self_isFirst-1) #最后根据是否先手判断正负
