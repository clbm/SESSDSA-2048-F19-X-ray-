Min_mark = -1<<20
Max_Mark = 1<<20
Dict = {1: 2, 2: 4, 3: 8, 4: 16, 5: 32, 6: 64, 7: 128, 8: 256, 9: 512, 10: 1024, 11: 2048}  #最后两个用于判断时计分
def getmark(board,Round):
    '评估函数'
    #part1:棋子直接估值部分
    #获得两方棋子数值的从小到大的列表
    global self_isFirst
    ...
    zeros = -len(ours)+len(theirs)           #用于比较空位数
    col = 4 - self_isFirst
    result = zeros + 3*board.getBelong((1,col)) + 3*board.getBelong((2,col))  #边界属于自己时，局面有利
    for chess in ours:
        result += Dict[chess]*(1+0.0003*Round)
    for chess in theirs:
        result -= Dict[chess]*(1+0.0003*Round)
    return result
...
def simulation(depth:int, Round:int, Gamemode:int, is_max:bool, board, alpha = Min_mark, beta = Max_Mark, is_root = False):
    ...
    if depth >= max_depth or Round == 500:
        alpha = beta = getmark(board,Round)
        return (alpha, beta)
...
class Player:
    ...
    def output(self, currentRound:int, board, mode:str):
        ...
        if mode == 'position':  # 给出己方下棋的位置
            Gamemode = 1^self_isFirst
            begain = 3-self_isFirst
        elif mode == 'direction':  # 给出己方合并的方向
            Gamemode = 2+(1^self_isFirst)
            begain = 1^self_isFirst
        else:
            return
        
        return simulation(depth = begain, Round = currentRound,Gamemode = Gamemode,is_max = True,board = board,is_root = True)
