...
Min_mark = -1<<20
Max_Mark = 1<<20
POS_2 = numpy.array([12,12,11,11,9,9,9,9])
def getmark(board):
    '评估函数'
    #part1:棋子直接估值部分
    #获得两方棋子数值的从大到小的列表
    global self_isFirst
    Value, Belong = numpy.array(board.getRaw()).transpose(2,0,1)
    Score_nozero = ((1<<Value)*(Value==0)*(21*Belong-POS_2)).sum()
    Score_zero = ((Value>0)*(2*Belong-1)).sum()
    if self_isFirst:
        return Score_nozero+Score_zero
    return -(Score_nozero+Score_zero)
